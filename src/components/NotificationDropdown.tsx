...
const NotificationDropdown = () => {
  ...
  const unreadCount = (unreadCountData?.unreadCount || 0) + socketActivities.length;
    ...
    <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
      ...
      >
            ...
            <DropdownMenuGroup>
              {allActivities.map((activity, index) => (
                <React.Fragment key={activity._id || `socket-${index}`}>
                  <DropdownMenuItem
                    className={`px-4 py-3 cursor-pointer focus:bg-primary/5 ${!activity.read ? 'bg-primary/5' : ''}`}
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm line-clamp-2 ${!activity.read ? 'font-semibold' : ''}`}>
                          {activity.message}
                        </p>
        ...
        <div className="border-t border-black/10 dark:border-white/10 p-2">
          ...
          >
            View All Notifications <ChevronRight className="h-3.5 w-3.5 ml-1" />
          </Button>
        </div>
      </DropdownMenuContent>
    </DropdownMenu>
